...
## stock group in month
{
  stock_group = read.csv('Results/stock_out_linear.csv')
  ...
  write.csv(stock_group_m, 'Results/stock_group_month_linear.csv', row.names = FALSE)
}
...
{
  tc_group = read.csv('Results/total_in_and_out.csv')
  date = as.Date(read.csv('date_EVaR.csv')[,1])
...
write.csv(tc_group_m, 'Results/tc_group_month.csv', row.names = FALSE)
...
## area in year
{
  area_in = read.csv('Results/area_in.csv')
  date = as.Date(read.csv('date_EVaR.csv')[,1])
...
write.csv(area_in_y, 'Results/area_in_year.csv', row.names = FALSE)
