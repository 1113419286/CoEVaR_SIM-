...
for (i in 1:30) {
  der.c[, , i] = insertCol(as.matrix(read.csv(file = paste("Results/beta_L", i, ".csv", 
                                                           sep = "")))[, 2:30], i, vec_zero)
}
...
save(con,file = "Results/co.RData")
...
for (j in 1:30) {
  lambda.firm      = read.csv(file = paste("Results/lambda_L", j, ".csv", sep = ""))
...
write.csv(tc_l, file = "Results/total_connectedness_and_averaged_lambda.csv", row.names = FALSE)


# the average coevar and evar
average_evar=apply(as.matrix(read.csv
                             ('Results/EVaR_movingwindows.csv')),2,'mean')
average_coevar=c()
for (j in 1:30) {
...
colnames(average_co_and_evar)=c('EVaR','CoEVaR_linear')
stock.name=as.data.frame(read_excel('name.xlsx'))
rownames(average_co_and_evar)=stock.name[,1]

write.csv(average_co_and_evar,"Results/averaged_evar_and_coevar.csv", row.names = FALSE)
...
### stock out
{
  stock_out = matrix(NA,rpd,cpd)
  colnames(stock_out) = stock.name[,1]
  
  for (i in 1:rpd) {
    stock_out[i,] = abs(apply(con[,,i],2,'sum'))
  }
  write.csv(stock_out, file = "Results/stock_out_linear.csv", row.names = FALSE)
}
...
colnames(tc_in) = c("Americas_in", "Asia_in", "Europe_in")
...
colnames(tc_out) = c("Americas_out", "Asia_out", "Europe_out")
tc_group = cbind(tc_in, tc_out)

# generate the necessory file for the quantlet TENET_total_in_out_groups
write.csv(tc_group, file = "Results/total_in_and_out.csv", row.names = FALSE)
...
## risk in divided by area
{
  ...
  write.csv(area.in_sim, file = "Results/area_in.csv", row.names = FALSE)
  
}



name.stock=as.data.frame(read_excel('name.xlsx'))
...
colnames(tot.ct) = name.stock[,1]
# generate the necessory file for the quantlets TENET_total_in_out_individual and
# TENET_SIFIs
write.csv(tot.ct, file = "Results/tot_c_overtime.csv", row.names = FALSE) 
