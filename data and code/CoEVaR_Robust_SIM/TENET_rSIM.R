...
#get date
{
  ...
  # write.csv(date0,'date_rlogre.csv',row.names = F)
  # write.csv(date1,'date_rEVaR.csv',row.names = F)
}
...
EVaR = as.matrix(read.csv("robust_results/rEVaR_movingwindows.csv"))[, 1:30]
...
for (k in 1:30) {
  ...
  write.csv(beta_sim, file   = paste("robust_results/beta_sim", k, ".csv", sep = ""))
  write.csv(Coevar_sim, file  = paste("robust_results/Coevar_sim", k, ".csv", sep = ""))
  write.csv(lambda_sim, file = paste("robust_results/lambda_sim", k, ".csv", sep = ""))
  write.csv(first_der, file = paste("robust_results/first_der", k, ".csv", sep = ""))
  write.csv(partial_der, file = paste("robust_results/partial_der", k, ".csv", sep = ""))
} 
