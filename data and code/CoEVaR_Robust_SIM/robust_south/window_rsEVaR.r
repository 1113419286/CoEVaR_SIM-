...
data=as.data.frame(read.csv('logre_rs.csv')[,-1])
...
data_m     = as.matrix(data[, 36:44])

# read the log returns of 50 financial institutions
data_y     = as.matrix(data[, 1:35])
...
# the moving window size is 48,equal to 1.5 years
winsize    = 48
...
write.csv(EVaR,file = "rs_results/rsEVaR_movingwindows.csv",row.names = F)
